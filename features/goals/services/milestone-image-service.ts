...
// Owner-private hero photos for achievement milestones. Mirrors the note-images
// pattern (features/entries/services/note-image-service.ts): private bucket,
...
/**
 * Uploads a milestone hero photo and returns its storage path. Store the path
 * on milestones.photo_url; resolve to a viewable URL with
...
export async function uploadMilestonePhoto(
  ...
  const storagePath = `${auth.user.id}/${milestoneId}/${randomId()}.${extensionForFile(file)}`;
...
export async function createSignedMilestonePhotoUrl(storagePath: string): Promise<string> {
...
export async function removeMilestonePhoto(storagePath: string): Promise<void> {
  try {
    await supabase.storage.from(MILESTONE_PHOTOS_BUCKET).remove([storagePath]);
  } catch {
