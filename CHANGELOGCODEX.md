...
### Fixed
- Fixed **OHARA Notes Version 1.1** reference jumps to resolve only the exact stable source ID, center it inside the actual editor viewport, allow repeated jumps, and apply a restrained temporary highlight without replacing the user's text selection or accidentally selecting adjacent embedded Goals.
- Fixed decorative-page margin clicks and whitespace-only ranges opening browser/editor selection state; actual text selection, keyboard editing, list/checklist behavior, and existing source-preserving reference removal remain unchanged.
- Fixed PDF image loss and silent formatting flattening by fetching fresh owner-authorized signed image URLs, embedding bounded aspect-ratio-preserving images, and explicitly surfacing inaccessible image failures instead of exporting incomplete documents.
- **Echo category-header hover stability:** neutralized the global web button scale only for active category headers/add controls, added a fixed-layout title/control split with `minWidth: 0`, single-line ellipsis, clipped overflow, and non-shrinking count/chevron/add controls, preventing hover growth or long titles from escaping the shelf surface.
- **`supabase/migrations/037_fix_entry_category_link_source.sql` and `features/entries/security.test.ts`:** fixed the production `replace_entry_relationships` function so category-only Note relationships provide the required `link_source` value, preventing categorized note saves from failing while leaving existing Echo/Entry data untouched; added focused regression coverage for the corrected insert shape.
- **`features/entries/components/EntryDetailScreen.tsx`:** clear the cold-load spinner when a freshly fetched legacy Echo deep link is cached, so `/echo?entryId=...` completes its redirect instead of remaining on an indefinite loading state.
- **`features/entries/components/EntriesLibrary.tsx`:** separated each Entry card's primary navigation control from its actions control, eliminating invalid nested buttons while preserving the Note/Reflection symbols, card navigation, pinning, and deletion behavior.
