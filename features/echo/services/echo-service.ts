...
export async function fetchDashboardLatestEntry(
  ...
  // Keep the dashboard aligned with the redesigned Entries workspace. The
  // client reads only the canonical summary columns and bounds plain text
  // immediately when mapping the dashboard DTO.
  const { data, error } = await supabase
    .from('entries')
