...
privacy, Circles-sharing, and native-client boundary.
...
owner-private unless the author separately creates a Circles post; that post
stores only a server-snapshotted title and author-written short description.
Reflection text and takeaways are never used to pre-fill that description.
