...
### Added
- Added **Goals V2.0 — Phase 2B release cutover hardening** in Migration 050: aggregate-only mapping verification plus service-role-only catch-up, authenticated legacy-write freeze, atomic finalization, and narrow privilege restoration for an explicitly authorized application rollback. The disposable PostgreSQL harness now proves late post-048 writes are caught exactly once before legacy mutations are frozen while canonical Task RPCs remain writable.
- Added **Goals V2.0 — Phase 2B Tasks & Activity Foundation** through append-only migrations 046–049: canonical owner-scoped `tasks`, versioned IANA-timezone `task_schedules`, durable `task_occurrences`, retry receipts, trusted mutation/reconciliation RPCs, idempotent Tracker/action-log backfill, and atomic New Phase Task continuity.
...
- Added **Goals V2.0 Phase 2A — Goal Lifecycle Foundation** through additive Migration 045: explicit `expired` state, trustworthy future `completed_at` / `archived_at` / `expired_at` transitions, owner-readable immutable deadline history, owner-scoped expiration reconciliation, same-ID deadline extension, and an atomic predecessor-archive/successor-create New Phase RPC in `supabase/migrations/045_goal_lifecycle_foundation.sql` and `types/supabase.ts`.
...
### Changed
- Refactored Migration 048's already validated legacy mapping statements into the reusable `run_tasks_legacy_catchup_v1` release function so the initial backfill and final post-deployment catch-up share one implementation and retain the same baseline, recurrence, provenance, and no-inference rules.
