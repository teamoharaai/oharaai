-- Migration 060: preserve weekly-count cadence and atomically transfer the
-- current public Goal slot during New Phase. The established owner lock,
-- eligibility, fresh occurrence generation, history and lineage are unchanged.
-- This replaces the deployed RPC; historical migrations are not rewritten.
...
begin
  ...
  -- Relinquish the unique public slot before inserting its replacement.
  -- v_previous retains the original visibility for the successor. All writes
  -- share this transaction: any later failure restores the predecessor too.
  if v_previous.visibility = 'public' then
    update public.goals set visibility = 'private' where id = v_previous.id;
  end if;
  ...
  loop
    ...
    if v_schedule.id is not null then
      ...
      insert into public.task_schedules (
        ...
        start_date, end_date, local_time, timezone, is_active, source, target_count
      ) values (
