...
import { assignGoalsToProject, assignProjectTask, createProjectComment, fetchOwnedGoalsForProjects, fetchProjectWorkspace, setEntryProjectShare } from '@/features/projects/services/project-service';
...
export default function ProjectDetailScreen() {
  ...
  function openManage() { if (!project) return; setFormError(null); setManageOpen(true); }
  ...
  const projectHeader = <Surface>
    <View style={{ gap: SPACE.sm }}><View style={{ alignItems: 'center', flexDirection: 'row', flexWrap: 'wrap', gap: SPACE.sm }}><Typography accessibilityRole="header" variant="heading">{project.title}</Typography><Badge label={project.mode === 'guide' ? 'OHARA Guide' : project.mode} variant="active" /></View>{project.description ? <Typography variant="body">{project.description}</Typography> : null}<Typography variant="caption">{activeGoals.length} active {activeGoals.length === 1 ? 'Goal' : 'Goals'} · {project.collaboration.members.length}/{3} people · Updated {new Date(lastActivity).toLocaleDateString()}</Typography><View style={{ flexDirection: 'row', flexWrap: 'wrap', gap: SPACE.sm }}>{project.collaboration.members.map((member) => <View key={member.userId} accessibilityLabel={`${member.displayName}, ${member.relationshipLabel || member.role}`} style={{ alignItems: 'center', backgroundColor: colors.background.selectedRow, borderColor: colors.border.accent, borderRadius: RADIUS.round, borderWidth: 1, height: 36, justifyContent: 'center', width: 36 }}><Typography variant="caption">{member.displayName.slice(0, 2).toUpperCase()}</Typography></View>)}</View></View>
    <View style={{ flexDirection: 'row', flexWrap: 'wrap', gap: SPACE.md }}><Button variant="secondary" onPress={() => openManage()}>Members</Button><Button accessibilityLabel="Vault" leftIcon={<VaultIcon color={colors.text.onAccent} size={18} />} onPress={() => switchMode('vault')}>Vault</Button><Button onPress={() => openManage()}>Manage Project ▾</Button></View>
  </Surface>;
  const currentGoalsCard = <Surface title="Current Goals">
    {activeGoals.length ? <View style={{ gap: SPACE.sm }}>{activeGoals.slice(0, 3).map((goal) => { const summary = momentumByGoal.get(goal.id); const delta = summary?.weeklyChange ?? 0; const tasks = project.taskPreviews.filter((task) => task.goalId === goal.id); const nextMilestone = goal.milestones.filter((milestone) => !milestone.completedAt && milestone.dueDate).sort((a, b) => a.dueDate!.getTime() - b.dueDate!.getTime())[0]; return <View key={goal.id} style={{ gap: SPACE.xs }}><ProjectGoalRow goal={goal} /><Typography variant="caption">Lead: {memberName.get(goal.projectLeadId ?? '') ?? 'Unassigned'}{summary?.displayedValue != null ? ` · Momentum ${summary.displayedValue} ${delta >= 0 ? '↗' : '↘'} ${delta >= 0 ? '+' : ''}${Math.round(delta * 10) / 10}` : ''}</Typography><Typography variant="caption">{tasks.length} Tasks need attention{goal.deadline ? ` · ${goal.deadline.toLocaleDateString()}` : ''}{nextMilestone ? ` · Next: ${nextMilestone.title}` : ''}</Typography></View>; })}</View> : <Typography variant="body">No active Goals yet.</Typography>}
  ...
  const entryCard = (kind: 'note' | 'reflection') => {
    ...
    return <Surface title={label}><Typography variant="caption">{items.length} authorized {items.length === 1 ? kind : `${label.toLowerCase()}`}</Typography>{items.slice(0, 2).map((entry) => <View key={entry.id} style={{ borderBottomColor: colors.border.divider, borderBottomWidth: 1, gap: SPACE.xs, paddingBottom: SPACE.md }}><Pressable accessibilityRole="button" onPress={() => router.push({ pathname: '/(app)/entries/[id]' as never, params: { id: entry.id } })}><Typography variant="emphasis-sm" numberOfLines={1}>{entry.title || 'Untitled'}</Typography>{entry.plainText ? <Typography variant="caption" numberOfLines={2}>{entry.plainText}</Typography> : null}</Pressable><Typography variant="caption">{entry.projectShareScope === 'guide' ? 'Shared with Guide' : entry.projectShareScope === 'project' ? 'Shared with Project' : 'Private to owner'}</Typography>{project.collaboration.role === 'owner' && entry.project?.id === project.id ? <Button size="compact" variant="secondary" onPress={() => void (async () => { setBusy(true); try { await setEntryProjectShare(entry.id, entry.projectShareScope === 'private' || !entry.projectShareScope ? (project.mode === 'guide' ? 'guide' : 'project') : 'private'); await load(); } finally { setBusy(false); } })()}>{entry.projectShareScope === 'private' || !entry.projectShareScope ? `Share with ${project.mode === 'guide' ? 'Guide' : 'Project'}` : 'Make private'}</Button> : null}</View>)}{!items.length ? <Typography variant="body-small">No authorized {label.toLowerCase()} yet.</Typography> : null}{actionLink(kind === 'note' ? 'Open Notes' : 'View reflections', () => switchMode('vault', kind === 'note' ? 'notes' : 'reflections'))}</Surface>;
  };
  const intelligenceCard = <Surface intelligence title="OHARA Intelligence"><Typography variant="caption">{insight.subtitle}</Typography><Typography variant="body">{insight.primary}</Typography>{insight.secondary ? <Typography variant="caption">{insight.secondary}</Typography> : null}</Surface>;
  const tasksCard = <Surface title="Project Tasks"><View style={{ flexDirection: 'row', flexWrap: 'wrap', gap: SPACE.sm }}>{(['mine', 'everyone', 'upcoming'] as const).map((filter) => <Button key={filter} size="compact" variant={taskFilter === filter ? 'primary' : 'secondary'} onPress={() => setTaskFilter(filter)}>{filter[0].toUpperCase() + filter.slice(1)}</Button>)}</View>{visibleTasks.length ? visibleTasks.slice(0, 5).map((task) => <View key={task.id} style={{ borderBottomColor: colors.border.divider, borderBottomWidth: 1, gap: SPACE.xs, paddingBottom: SPACE.md }}><Pressable accessibilityRole="button" onPress={() => router.push(`/(app)/goals/${task.goalId}` as never)} style={({ pressed }) => ({ opacity: pressed ? 0.7 : 1 })}><View style={{ alignItems: 'center', flexDirection: 'row', gap: SPACE.sm }}><Ionicons color={task.timing === 'overdue' ? colors.feedback.danger.text : colors.accent.primary} name="ellipse-outline" size={16} /><Typography variant="emphasis-sm" numberOfLines={1} style={{ flex: 1 }}>{task.title}</Typography><Typography variant="caption">{task.timing === 'anytime' ? 'Anytime' : task.timing[0].toUpperCase() + task.timing.slice(1)}</Typography></View><Typography variant="caption">{task.assignedTo ? memberName.get(task.assignedTo) ?? 'Assigned member' : 'Unassigned'} · From: {task.goalTitle}</Typography></Pressable><View style={{ flexDirection: 'row', flexWrap: 'wrap', gap: SPACE.xs }}>{capabilities.has('assign_task') ? project.collaboration.members.map((member) => <Button key={member.userId} size="compact" variant={task.assignedTo === member.userId ? 'primary' : 'secondary'} onPress={() => void (async () => { await assignProjectTask(task.id, member.userId); await load(); })()}>{member.displayName}</Button>) : null}{capabilities.has('comment') ? <Button size="compact" variant="secondary" onPress={() => { setCommentTask({ id: task.id, title: task.title }); setCommentBody(''); }}>Comment</Button> : null}</View></View>) : <Typography variant="body-small">No Tasks match this view.</Typography>}{project.taskPreviews.length ? actionLink('Open Goal Tasks', () => router.push(`/(app)/goals/${project.taskPreviews[0].goalId}` as never)) : null}</Surface>;
  ...
  return <>
    ...
    <ManageProjectModal visible={manageOpen} project={project} onClose={() => setManageOpen(false)} onReload={load} onOpenGoalPicker={() => void openGoalPicker()} />
