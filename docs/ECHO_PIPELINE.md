...
- Social features never grant friends read access to `public.entries`. If a
  Reflection is explicitly referenced by a separate social post, that post must
  carry only an author-approved snapshot; it must not expose or pre-fill the
  Reflection body, takeaway, BRT, Goal/Project links, or later edits.
- Notes are not a social-sharing surface.
