...
### Added
- Added **Tracker Metrics — Task 10: automated + manual verification / release gate (initiative complete).** Added the `test:tracker-metrics` package script (`node --experimental-strip-types --test`, mirroring `test:momentum`/`test:constellation`) covering the full suite — file list verified with `rg`/`ls`, not hardcoded: `lib/goals/tracker-cadence.test.ts`, `lib/goals/tracker-period.test.ts`, `lib/goals/due-today.test.ts`, `lib/db/paginate.test.ts`, `lib/db/tracker-mutations.test.ts`, and `features/goals/tracker-optimism.test.ts`, `tracker-display.test.ts`, `tracker-grouping.test.ts`, `tracker-card-display.test.ts`, `tracker-boundary.test.ts`, `tracker-detail-state.test.ts`, `dashboard-due-today.test.ts`. **Full automated matrix green:** `npx tsc --noEmit` clean (before and after); `npm run test:tracker-metrics` 116/116; `npm run test:momentum` 64/64; all `features/goals/*.test.ts` (incl. non-tracker goals suites) 64/64; `npm run test:ios-contract` 4/4. The **>1,000-log fixture** is confirmed to prove full sum/history with no API row-ceiling truncation (`lib/db/paginate.test.ts:47` → 1,500 logs sum to the full derived `currentValue`; `lib/db/tracker-mutations.test.ts:335` → 1,500×2 = 3,000 prior-phase achieved). Completed the **`current_value`/`currentValue` reference audit** (full table in the changelog): no tracker *period UI* reads `trackers.current_value`; the only non-derived reads left are (a) the accepted `HomeGoalPreview` `nextTracker` label picker at `app/(app)/dashboard.tsx:612` on the un-hydrated goal-list path (where `periodState` is never populated) plus its `goal-service` hydration into the `Tracker.currentValue` field, and the **dead** `updateTrackerValue` store action (`features/goals/store.ts:53`, zero callers); every remaining write is a `NOT NULL DEFAULT 0` insert default; all display/derivation references use `periodState`, generated types, or comments. Re-confirmed the prior deletion findings hold: the legacy `app/api/goals/complete-tracker+api.ts` route is absent, the `completeTracker` wrapper is gone from `lib/db/goals.ts`, and `TrackerList` is gone from `GoalsWorkspace.tsx` (shared `logTrackerMutation`/`createTrackerMutationDb` retained). Mapped the full **manual matrix** (counter/habit/checklist × daily/weekly/monthly; counter below/at/above target; seven-bucket positioning; device-tz≠profile-tz; DST spring-forward/fall-back per the D-005 determinism caveat; null-cadence→assign→log; uncomplete-clears-all; SQL prior-period backdate does not complete the current period; expired/successor read-only; extension prior-phase summaries) to its automated coverage — every settled semantic is unit-covered; the live column (UI taps, direct-SQL backdate, a second device timezone, real DST wall-clock) needs an authed live environment or the user and is not app-drivable headless. No product-source behavior changed (the audit surfaced no bug); the two documented non-derived references are accepted/deferred per new decision **D-012**. **Release gate: SHIP** for the whole tracker-metrics initiative (Tasks 0–10). See `tracker-metrics/changelog/010-verification-release-gate.md`.
- Added **Tracker Metrics — Task 9: dashboard daily-state alignment + legacy-route deletion.** Migrated the dashboard "Today" due-today card off the legacy `POST /api/goals/complete-tracker` route onto the shared `POST /api/trackers/log`, made `GET /api/trackers/due-today` timezone-aware, and (once `rg` confirmed no remaining caller) **deleted** the legacy route file `app/api/goals/complete-tracker+api.ts` and the `completeTracker` wrapper in `lib/db/goals.ts` — keeping the shared mutation core/adapter (`logTrackerMutation`, `createTrackerMutationDb`). New pure, node-tested `lib/goals/due-today.ts` (relative imports — D-004): `deriveDueTodayState(meta, logs, timezone, asOf)` derives the CURRENT daily window via the shared `getPeriodBounds('daily', …)` and returns `{ currentPeriodValue, isCompletedThisPeriod, periodEndExclusive }` with the settled completion semantics (counter: current-period sum ≥ a positive `targetValue`; habit/checklist: any presence). `app/api/trackers/due-today+api.ts` rewritten: reads `profiles.timezone` (normalized via `normalizeTimezone`, invalid/missing → UTC), captures one `asOf`, derives one daily half-open window in that zone, queries only daily trackers and **only that window's** `tracker_logs` (`fetchAllPages` with `gte`/`lt` — never full history), and returns per-tracker `currentPeriodValue`/`isCompletedThisPeriod`/`periodEndExclusive`, dropping the stale `current_value` and `lastCompletedAt`. `app/(app)/dashboard.tsx` `DueTodayZone`: removed the browser-timezone `isCompletedToday` date comparison and all `lastCompletedAt` handling; consumes the server boolean; `handleComplete` posts `/api/trackers/log` with the action chosen by tracker type (`counter → 'counter-log'` so a `+1` below target does NOT mark complete, `habit`/`checklist` → `'complete'`) and reconciles completion from `payload.periodState.isCompleted`; added a stable `load` (`useCallback`) + `isMountedRef` guard and wired `useTrackerBoundaryRefresh` over minimal `periodEndExclusive`-carrying stand-ins so the zone refreshes at user-local midnight and on foreground/visibility/focus (mirrors goal detail). The dashboard card stays one-way (no uncomplete gesture — that remains a goal-detail concern). `docs/API_CONTRACT.md`: replaced the deprecated complete-tracker section with a "Removed" note and rewrote the due-today section to the booleans/`periodEndExclusive`/timezone shape. Added `lib/goals/due-today.test.ts` (8 pure: tz window incl. a Tokyo cross-UTC-day case, invalid-tz→UTC fallback, habit/checklist presence, counter target boundary, zero/null target never completes, out-of-window logs ignored, DST spring-forward determinism per D-005) and `features/goals/dashboard-due-today.test.ts` (6 text-level: dashboard on the shared route, action-by-type, periodState reconcile with no client-clock helper, boundary refresh, tz-aware booleans-not-scalars route, and an assertion that the legacy route file + `completeTracker` wrapper are gone while the core remains). `npx tsc --noEmit` clean before and after; goals + lib tracker suites 129/129; `npm run test:momentum` 64/64. New decision **D-011** (dashboard action-by-type, one-way card, boundary-refresh reuse; confirms the D-009 deletion condition met). Pre-existing legacy read flagged for the Task 10 audit: `HomeGoalPreview` still reads `tracker.currentValue` on the goal-list path. See `tracker-metrics/changelog/009-dashboard-daily-state.md`.
- Added **Tracker Metrics — Task 8: tracker card + habit-history rewrite.** The tracker card DISPLAY is finally driven off the log-derived `tracker.periodState`, retiring the interim legacy-scalar (`trackers.current_value`) read that D-007/D-008 left in place. New pure, node-tested `features/goals/tracker-display.ts` (relative imports + `import type` only — D-004) owns the extractable display logic: `counterProgressPercent(currentValue, targetValue)` (0–100 clamp; missing/zero target → denominator 1 so any progress fills; non-finite → 0), `currentPeriodValue`/`isTrackerPeriodComplete` (null-safe — a null/unhydrated `periodState` is never authoritative), `habitBucketViews(periodState, frequency)` (exactly seven `recentPeriods` dots oldest→newest with the current period last, trusting the `deriveTrackerPeriodState` ordering; pads to seven empty placeholders with unique negative keys + "No history yet" when `periodState` is null; slices to the newest seven if handed more), and `bucketPeriodLabel` (a date "Sep 11" for daily/null, "Week of Sep 11" for weekly, "September 2026" for monthly via `Intl.DateTimeFormat` in the period-state timezone; the a11y label appends the current-period marker + `logged`/`no log`). `features/goals/components/TrackerCard.tsx`: removed the `displayValue` local state + its `useEffect` (the last scalar-drift path); the counter number/progress bar, habit dots (same 26px size + `brt.rose`/`border.warmSubtle` tokens, now each with an accessible bucket label), and checklist checked/strike-through/label (from `isCompleted` **only** — dropped the `|| displayValue >= target` fallback) all read `periodState`. The habit/checklist one-tap control is now an accessible **checkbox toggle** (`accessibilityRole="checkbox"` + `accessibilityState={{ checked: isCompleted, disabled }}`) that logs when unchecked and calls the new `onLogUncomplete` (`useGoalDetail.onUncompleteTracker`) when checked, disabled when the applicable-direction handler is absent — a discrete control, so the editable/deletable card is never an undo target. `onLogUncomplete` is threaded `TrackersPanel` → `TrackerCard` (Task 7 grouping/keys untouched, so a card keeps its instance + `isSaving` across group moves), and `GoalsWorkspace` wires `onLogUncomplete={goalDetail.onUncompleteTracker}` at both `TrackersPanel` call sites. Confirmed via `rg` and **removed the dead `TrackerList`** function in `GoalsWorkspace.tsx` (no JSX call site) plus its now-unused `Tracker` type import. Added `features/goals/tracker-display.test.ts` (9 pure: clamp below/at/above + missing/zero/non-finite target; null value/completion; seven-bucket current-last positioning + ascending order; daily-logged-today fills the current bucket only; weekly/monthly current-last; null→seven empty placeholders with unique keys; >7→newest seven; date/week/month labels + a11y markers) and `features/goals/tracker-card-display.test.ts` (6 text-level: card off the helpers with no `displayValue`; habit uses `habitBucketViews` not `dotCount`/`filledDots`; checklist strike from `isCompleted`; checkbox toggle role/checked + `onLogUncomplete` selection; `onUncompleteTracker` threaded hook→panel→card; `TrackerList` + import removed); updated `tracker-detail-state.test.ts` to assert the extracted `isTrackerPeriodComplete(tracker.periodState)` helper. `npx tsc --noEmit` clean before and after; all `features/goals/*` 58/58; `npm run test:momentum` 64/64. New decision **D-010** (periodState is the sole display source, incl. the null/unhydrated empty fallback: counter shows 0, habit shows seven empty dots — no legacy-scalar read). **Interim caveat from Tasks 5–7 now RESOLVED:** the card no longer reads `trackers.current_value`. Locked scope respected: no dashboard / `due-today` / `complete-tracker` changes (Task 9, D-009). See `tracker-metrics/changelog/008-task8-card-habit-history-rewrite.md`.
- Added **Tracker Metrics — Task 7: Ongoing/Completed tracker grouping.** `features/goals/components/TrackersPanel.tsx` now partitions the goal-detail trackers into **Ongoing** and **Completed** sections driven by the log-derived `tracker.periodState?.isCompleted ?? false` (Task 6's DB-derived completion), replacing the single flat `sortedTrackers` list. Extracted the partition as a pure, node-tested `partitionTrackersByCompletion(trackers)` in new `features/goals/tracker-grouping.ts` (relative imports — D-004; also exports `isTrackerCompleted`): it sorts by `sortOrder` once and splits into `{ ongoing, completed }`, preserving ascending `sortOrder` within each section. **Null-cadence and not-yet-hydrated trackers (`periodState === null`) stay in Ongoing** and keep the existing "Cadence not set" affordance — null is never treated as complete. The panel calls the helper inside a `useMemo` keyed on the `trackers` array so typing in the add-form (or any unrelated re-render) does not re-copy/re-sort. Each section header ("Ongoing"/"Completed") renders **only when that section is non-empty** (`accessibilityRole="header"` + `accessibilityLabel="… trackers"`, reusing the file's existing overline typography/scale); the first rendered header gets a small top margin, the second an 18px group gap. All cards **and** headers are siblings in one parent keyed by `tracker.id`, so a card that moves between sections (e.g. an optimistic complete flips `isCompleted` immediately) keeps its React instance and its in-flight `isSaving` affordance rather than unmounting/remounting. Locked scope: no card/habit-history rewrite, no counter/checklist display driven off `periodState`, no uncomplete gesture, no dashboard/legacy-route changes (Tasks 8/9). Added `features/goals/tracker-grouping.test.ts` (10: `isTrackerCompleted` null/true/false; checklist+habit-with-log→Completed; counter below→Ongoing, at/above→Completed; null-cadence→Ongoing; empty section→empty array/no header; `sortOrder` preserved per section; no input mutation; plus text-level assertions that TrackersPanel uses the memoized helper, dropped `sortedTrackers`, and renders conditional accessible headers). `npx tsc --noEmit` clean before and after; grouping 10/10; Task 6 suites (`tracker-optimism`/`tracker-boundary`/`tracker-detail-state`) 20/20; all `features/goals/*` 43/43; `npm run test:momentum` 64/64. **Interim (unchanged):** counter/checklist visible display still reads the legacy scalar until Task 8; `onUncompleteTracker` still has no card gesture until Task 8. See `tracker-metrics/changelog/007-task7-ongoing-completed-grouping.md`.
- Added **Tracker Metrics — Task 6: goal-detail state, optimistic mutations, boundary refresh.** Deleted the local `completedTrackerIds` React set (hook state + reset effect + `UseGoalDetailResult` member + `TrackersPanel.completedIds` prop + both `GoalsWorkspace` call sites) so completion is DB-derived: `TrackerCard` now reads `tracker.periodState?.isCompleted ?? false` (its `isCompleted` prop removed; a null/unhydrated state is never presented as authoritative incomplete). All three goal-detail tracker mutations go through the shared authenticated `POST /api/trackers/log` route and reconcile from the returned `periodState` via `periodStateFromDto`+`patchTracker`: `onCompleteTracker` (`action:'complete'`, drops the legacy route + the `success`-only assertion + the `currentValue:1` write, idempotent skip when already completed), `onLogCounter` (adds optimistic +1 bucket bump on top of the Task-5 reconcile), and new `onUncompleteTracker` (`action:'uncomplete'`, optimistic clear; counters short-circuited). New pure, node-tested `features/goals/tracker-optimism.ts` (relative imports — D-004): immutable `applyOptimisticComplete/Uncomplete/Counter` current-bucket transforms, `completionValueForTracker` (mirrors the server rule), the `patchTrackerInGoals` functional merge-by-id reducer, and a per-tracker mutation registry (`beginMutation`→seq or `null` when an identical action is in flight = one logical completion on rapid double-tap; `isLatestMutation` ordering guard; `endMutation`; `resetRegistry`). Rollback restores the prior `periodState` **only if the request is still the latest** for that tracker, so complete-then-immediate-uncomplete applies responses order-independently. `features/goals/store.ts` gains `patchTracker(goalId, trackerId, patch)` (functional merge, never a whole-object clobber of unrelated fields/trackers) used for every optimistic/reconcile/rollback write; `upsertTracker` retained for add/delete. Cadence-boundary refresh: pure `features/goals/tracker-boundary.ts` (`earliestConfiguredPeriodEndEpoch`, `computeBoundaryDelayMs`) + `features/goals/hooks/useTrackerBoundaryRefresh.ts` (timer fired just after the earliest configured `periodState.endExclusive`, recomputed via a memoized boundary epoch so optimistic patches don't churn it, cleared on goal change, **plus** RN `AppState` `active` and web `visibilitychange`/`focus` — never the timer alone). `useGoalDetail.onSaveTracker` now merges metadata via `patchTracker` (preserving the live `periodState`) and, on a frequency/target change, drops the now-invalid derivation (`periodState:null`) and calls the new stable `refreshDetail()` to rederive; `refreshDetail` reads the store via `getState()` so it stays reference-stable for the boundary effect, and reuses `mergeServerGoal` to keep enrichment/successor fields. Added `features/goals/tracker-optimism.test.ts` (15), `features/goals/tracker-boundary.test.ts` (4), and text-level `features/goals/tracker-detail-state.test.ts` (5: no completion set anywhere, all three actions on `/api/trackers/log`, optimism+guards present, frequency/target invalidation→rederive, boundary-hook wiring). `npx tsc --noEmit` clean before and after; new suites 24/24; Task 3/4/5 suites 57/57; `features/goals/*` + `refresh` 16/16; `npm run test:momentum` 64/64. **Legacy route deferred (D-009):** `app/api/goals/complete-tracker` is now deprecated and unused by goal detail, but the dashboard due-today card still calls it, so its deletion (route file + `completeTracker` wrapper) moves to Task 9 which migrates the dashboard — documented in `docs/API_CONTRACT.md`. **Interim (unchanged):** counter/checklist visible display still reads the legacy scalar until Task 8; `onUncompleteTracker` has no card gesture until Task 8 wires the accessible toggle. See D-009 + `tracker-metrics/changelog/006-task6-detail-state-optimism.md`.
- Added **iOS beta Task 7 shared contract/security hardening.** Documented every native API/table/RPC/auth operation and its side effects in `docs/IOS_API_CONTRACT_V1.md`; added versioned Momentum/Goals/Entries fixtures and `test:ios-contract` coverage for error/request-ID helpers and owner policies through Migration 046. The native routes now return request-correlated, no-store structured errors; distinguish 400/401/403/404/409/422/429/5xx; validate owner-scoped identifiers; avoid existence disclosure; and emit only PII-safe diagnostics. Removed raw goal titles, owner/record IDs, Entry content, and database messages from the audited paths. Service-role Momentum publication remains server-only. Goal creation and tracker-completion idempotency/transaction gaps remain explicitly documented risks.
- Added **Tracker Metrics — Task 5: authenticated logging/uncomplete + legacy-consumer fixes.** Refactored `completeTracker` into a shared authenticated tracker-log mutation and restored the counter `+1` inert since D-007. New `lib/db/tracker-mutations.ts` (relative imports — D-004) holds the pure orchestration `mutateTrackerLog(input, db, asOf)` behind a narrow injected `TrackerMutationDb` port: it enforces successor/read-only → goal ownership → tracker-in-goal → action/type compatibility → null-cadence rejection, then does ONE bounded seven-period `tracker_logs` read and derives the authoritative period-state DTO from the resulting log set with a single captured `asOf` (no re-read). Actions: `complete` (checklist/habit, **idempotent** — no second current-period log; checklist value 1, habit positive `targetValue` else 1), `counter-log` (counter only, finite-positive value, default 1), `uncomplete` (checklist/habit, deletes **all** current-period logs so duplicates clear; rejected for counters). It **never writes `trackers.current_value`**. Errors are a typed `TrackerMutationError` mapped to HTTP by `trackerMutationErrorStatus` (successor→409, not-found→404, null-cadence→422, invalid action/value→400). The Supabase-backed adapter `createTrackerMutationDb` (paginated `fetchPeriodLogs` via `fetchAllPages`, scoped `deleteLogsInRange`) and the entry point `logTrackerMutation` live in `lib/db/goals.ts`; `completeTracker` now delegates and returns `{ success: true, periodState }` (checklist `current_value=1` write removed). New single actioned route `app/api/trackers/log+api.ts` (`{trackerId, goalId, action, value?}`); the legacy `app/api/goals/complete-tracker+api.ts` returns the same additive DTO and maps `TrackerMutationError` (so the existing `onCompleteTracker` `success`-only assertion is untouched — Task 6 consumes `periodState`). `cloneGoalWithMilestonesAndTrackers` prior-phase summaries now derive from a **paginated** phase-window log read across all tracker types via the pure `lib/goals/phase-summary.ts` (counter = sum of log values; habit/checklist = count of logs, replacing the stale `current_value>0?1:0` checklist rule); dropped `current_value` from the clone select. Client: `goal-service.periodStateFromDto` (ISO→Date boundary); `useGoalDetail.onLogCounter` posts `action:'counter-log'` and reconciles `periodState` (non-optimistic — Task 6 adds optimism/in-flight guard/boundary refresh); `TrackerCard.increment()` re-wired to `onLogCounter`, the manual current-progress input removed, and the `✓ Log` one-tap button hidden for counters; `onLogCounter` threaded through `TrackersPanel`/`GoalsWorkspace`; `ExtendGoalModal` reads `periodState` (not the stale `currentValue`). Documented `POST /api/trackers/log` (actions/semantics/DTO), log-derived completion, and reversible habit/checklist state in `docs/API_CONTRACT.md`. Added `lib/db/tracker-mutations.test.ts` (19: ownership/successor/not-found rejection, null-cadence rejection for all actions, action/type mismatches, complete value + idempotency + previous-period insert, two-tap counter sum, counter target, value validation, uncomplete-deletes-all + cannot-reach-previous-period, DTO shape, error→status, prior-phase summary incl. a >1000-log fixture). `npx tsc --noEmit` clean before and after; tracker suites 57/57; `npm run test:momentum` 64/64; goals + refresh suites 16/16. **Interim:** counter `+1` logs correctly and updates store `periodState`, but the visible counter number waits on Task 8 (card display still reads the legacy scalar) — do not ship before Task 8. See D-008 + `tracker-metrics/changelog/005-task5-auth-logging-uncomplete.md`.
- Added **Tracker Metrics — Tasks 3+4: tracker period contract + derivation (atomic).** Added `TrackerPeriodBucket`/`TrackerPeriodState` and `Tracker.periodState: TrackerPeriodState | null` to `features/goals/types.ts`, and **removed `currentValue` from `TrackerUpdates`** (period progress is derived from `tracker_logs`, no longer client-writable — see D-007). New `lib/goals/tracker-period.ts` is a pure, timezone-aware `deriveTrackerPeriodState(meta, logs, tz, asOf)`: it buckets logs into the seven most recent cadence periods (oldest→newest, current last) via the Task-1 cadence utilities, sums per bucket and current period on half-open bounds, and applies settled completion semantics (checklist/habit = ≥1 current-period log; counter = current-period sum ≥ a positive `targetValue`), returning `null` for null-cadence trackers. New `lib/db/paginate.ts` adds a generic `fetchAllPages(fetchPage, pageSize=500)` mirroring the `lib/db/friends.ts`/`constellation.ts` loop (throws on any page error so a partial read never looks complete). `features/goals/services/goal-service.ts` gains the hydrated goal-detail read path (`fetchHydratedGoalDetail`/`hydrateGoalTrackers`): it reads the authed user's `profiles.timezone`, captures one `asOf`, partitions trackers by cadence, and issues **at most three parallel** paginated `tracker_logs` reads (one per non-empty cadence, each using that cadence's exact seven-period lower bound — a monthly tracker never forces seven months of daily logs), selecting only `id, tracker_id, value, logged_at` ordered by `logged_at` then `id`; on a log-read failure it surfaces a tracker-state load error instead of mapping missing evidence to zero. `mapTracker` initializes `periodState: null`, and `useGoalDetail` now always hydrates the **selected goal only** on open (a listed goal is no longer treated as already-hydrated). `TrackerCard` received compile-safety-only edits for the `TrackerUpdates` change (counter `+1` is temporarily inert — Task 5 restores it via authenticated logging; D-007). Added `lib/goals/tracker-period.test.ts` (20) and `lib/db/paginate.test.ts` (5, incl. a >1000-log pagination→correct-sum fixture). `npx tsc --noEmit` clean before and after; `npm run test:momentum` 64/64; cadence 13/13; existing goals suites 9/9. Uses relative imports in test-reachable modules (D-004). See `tracker-metrics/changelog/004-tasks3-4-period-derivation.md`.
